...
Postman.prototype._mapEndpoint = function(pmr) {
  ...
  endpoint.Id = pmr.id || pmr.request.url;
  urlParts = pmr.request.url.split('?');
  ...
  endpoint.PathParams = this._mapURIParams(pmr.request.pathVariables);

  //parse headers
  endpoint.Headers = this._mapRequestHeaders(pmr.request.header);
  //TODO map Body
  endpoint.Consumes = mapConsumes(pmr.request.dataMode);

  if (pmr.request.body && !_.isEmpty(pmr.request.body.raw)) {
    // is it parsable?
    var data = pmr.request.body.raw;
    ...
    // save the example
    var body = {
      example: data || pmr.request.body.raw,
    }
...
Postman.prototype._mapSavedEntry = function(pmr) {
  if(!pmr.request.url) return null;

  var savedEntry = new SavedEntry(pmr.name);
  var urlParts = pmr.request.url.split('?');

  savedEntry.Id = pmr.id || urlParts;
  ...
  savedEntry.PathParams = this._mapURIParams(pmr.request.pathVariables);
  savedEntry.Headers = this._mapRequestHeaders(pmr.request.header);
  savedEntry.Consumes = mapConsumes(pmr.request.dataMode);
  if (savedEntry.Method.toLowerCase() !== 'get' &&
      savedEntry.Method.toLowerCase() !== 'head') {
    savedEntry.Body = this._mapRequestBody(pmr.request.body.raw);
  }
...
Postman.prototype._import = function() {
  ...
  var requests = this.data.requests || this.data.item || [];
  var folders = this.data.folders || this.data.info || [];
